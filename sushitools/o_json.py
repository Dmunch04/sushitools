...
__JSON_TYPE_EASY_MAP = {
    JSONType.TRUE: lambda val: "true",
    JSONType.FALSE: lambda val: "false",
    JSONType.NULL: lambda val: "null",
    JSONType.INTEGER: lambda val: val.integer,
    JSONType.FLOAT: lambda val: val.floating
}

__JSON_TYPE_MAP = {
    JSONType.OBJECT: lambda val: "{{}}"
}


def other_to_json(root: JSONValue, pretty: bool = False) -> str:
    #json: str = ""
    json: list[str] = []

    eol: str = '\n' if pretty else ''
    _indent = "    " if pretty else ''
    write_eol = (lambda: json.append(eol)) if pretty else (lambda: ...)
    write_tabs = (lambda amount: json.append(_indent * amount)) if pretty else (lambda amount: ...)
    write_char_eol = lambda char: (json.append(char), write_eol())

    def write_string(string: str):
        nonlocal json
        global __ESCAPE_MAP
        json.append('"')
        esc_map = __ESCAPE_MAP
        json.extend([esc_map.get(char, char) for char in string])
        #for char in string:
            #json.append(esc_map.get(char, char))
        json.append('"')

    def write_value(value: JSONValue, indent_level: int, JSONType = JSONType):
        nonlocal json
        global __JSON_TYPE_EASY_MAP
        jt_map = __JSON_TYPE_EASY_MAP

        """
        def put_tabs(extra_indent: int):
            nonlocal json
            if pretty:
                json.append("    " * (indent_level + extra_indent))
        ...
        if tt == JSONType.OBJECT:
            ...
            if not obj:
                ...
                for name in obj:
                    ...
                    first = False
                    #put_tabs(1)
                    write_tabs(indent_level + 1)
                    write_string(name)
                    json.append(':')
                    if pretty:
                        json.append(' ')
                    write_value(val, indent_level + 1)

                #put_eol()
                write_eol()
                #put_tabs(indent_level)
                write_tabs(indent_level)
                json.append('}')
            ...
            if not arr:
                ...
                for i in range(len(arr)):
                    ...
                    #put_tabs(1)
                    write_tabs(indent_level + 1)
                    write_value(elem, indent_level + 1)
                ...
                write_tabs(indent_level)
                json.append(']')
        elif tt == JSONType.STRING:
            write_string(value.get_value())
        else:
            jt_map.get(tt)(value)
            """
            res = __JSON_TYPE_EASY_MAP.get(value.type_tag)
            if res:
                json.append(res)
            else:
                json.append(str(value.value))
            """
        """
                elif value.type_tag == JSONType.INTEGER:
                    json += str(value.integer)
                elif value.type_tag == JSONType.FLOAT:
                    json += str(value.floating)
                elif value.type_tag == JSONType.TRUE:
                    json += "true"
                elif value.type_tag == JSONType.FALSE:
                    json += "false"
                elif value.type_tag == JSONType.NULL:
                    json += "null"
                """
